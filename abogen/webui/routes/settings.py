...
def update_settings() -> ResponseReturnValue:
    ...
    try:
        current["supertonic_total_steps"] = max(
            2,
            min(
                15,
                int(
                    form.get(
                        ...
                        current.get("supertonic_total_steps", 5),
                    )
                ),
            ),
        )
    ...
    try:
        current["silence_between_chapters"] = max(
            0.0, float(form.get("silence_between_chapters", 2.0))
        )
    except ValueError:
        pass
    ...
    current["normalize_chapter_opening_caps"] = coerce_bool(
        form.get("normalize_chapter_opening_caps"), True
    )
    current["auto_prefix_chapter_titles"] = coerce_bool(
        form.get("auto_prefix_chapter_titles"), True
    )
...
def settings_page() -> str | ResponseReturnValue:
    ...
    if debug_run_id:
        run_dir = (
            Path(current_app.config.get("OUTPUT_FOLDER") or get_user_output_path("web"))
            / "debug"
            / debug_run_id
        )
