...
class TestSelectionHeuristics(unittest.TestCase):
    def test_pyqt_heuristics(self):
        ...
        cases = [
            ("Chapter 110: The Starlit Blueprint Crystal and the Map That Drew Our Shared Tomorrow!", False),
    ...
    def test_webui_heuristics(self):
        ...
        self.assertTrue(should_preselect_chapter(title, text, 0, 10), "Long map chapter should be selected")
        
        # Case 2: Just "Map" with no text
        title = "Map"
        text = ""
        # Score = 1.0 (map) + 0.9 (short text) = 1.9. 1.9 < 1.9 is False.
        self.assertFalse(should_preselect_chapter(title, text, 0, 10), "Standalone Map with no text should be de-selected")
        ...
        # Score = 1.0 (map)
        self.assertTrue(should_preselect_chapter(title, text, 0, 10), "Long title with Map but no 'Chapter' should be selected")

        # Case 4: Acknowledgments (short)
        title = "Acknowledgments"
        text = "Thanks to everyone." # < 150 chars
        # Score = 2.0 (ack) + 0.9 (short) = 2.9
        self.assertFalse(should_preselect_chapter(title, text, 0, 10), "Short acknowledgments should be de-selected")
